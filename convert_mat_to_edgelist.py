...
es = sorted(e,key=lambda x: x[2]['weight'])

G2 = nx.Graph()

for i in range(100):
    G2.add_edge(es[i][0],es[i][1],weight=es[i][2]['weight'])
    
for i in range(len(e)):
    e[i][2]['weight'] = abs(e[i][2]['weight'])
    
eas = sorted(e,key=lambda x: x[2]['weight'],reverse=True)

nx.draw(G)
pos = nx.spring_layout(G2)
edgewidth = [ d['weight'] for (u,v,d) in G2.edges(data=True)]
nx.draw_networkx_edges(G2, pos, width=edgewidth)